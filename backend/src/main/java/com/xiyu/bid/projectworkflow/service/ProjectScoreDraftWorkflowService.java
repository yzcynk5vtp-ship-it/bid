...
class ProjectScoreDraftWorkflowService {
    ...
    private RuntimeException toScoreDraftRuleException(ScoreDraftPolicy.RuleFailure failure) {
        return switch (failure) {
            case GENERATED_NOT_EDITABLE -> new RetryableOperationException(
                    409,
                    HttpStatus.CONFLICT,
                    "已生成正式任务的草稿不可修改"
            );
            case READY_REQUIRES_ASSIGNEE -> new InvalidArgumentException("生成正式任务前必须指定责任人");
            case ONLY_READY_DRAFTS_CAN_GENERATE_TASKS -> new InvalidArgumentException("仅 READY 状态的评分草稿可生成正式任务");
        };
    }
