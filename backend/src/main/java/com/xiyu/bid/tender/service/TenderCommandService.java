...
public class TenderCommandService {
    ...
    public TenderBidResponse proceedToBid(Long tenderId, Long userId) {
        ...
        // 从标讯评估表带入评估数据到项目立项（非阻塞，失败仅记日志）
        try {
            tenderEvaluationRepository.findByTenderId(tenderId).ifPresent(eval -> {
                var basic = eval.getBasic();
                ...
                ProjectInitiationDetails details = ProjectInitiationDetails.builder()
                        ...
                        .build();
                // 一、基础信息段字段映射
                if (basic.getPlannedShortlistedCount() != null) {
                    details.setExpectedBidders(basic.getPlannedShortlistedCount());
                }
                if (basic.getMroOfficeFlowAmount() != null) {
                    details.setAnnualEcommerceAmount(basic.getMroOfficeFlowAmount());
                }
                if (basic.getCustomerRevenue() != null) {
                    details.setAnnualRevenue(basic.getCustomerRevenue());
                }
                if (basic.getUnfavorableItems() != null) {
                    details.setTenderAdverseItems(basic.getUnfavorableItems());
                }
                if (basic.getRiskAssessment() != null) {
                    details.setRiskAssessment(basic.getRiskAssessment());
                }
                if (basic.getContingencyPlan() != null) {
                    details.setRiskMitigationPlan(basic.getContingencyPlan());
                }
                if (basic.getProcessKnowledge() != null) {
                    details.setPmUnderstandsProcess(basic.getProcessKnowledge());
                }
                if (basic.getSupportNotes() != null) {
                    details.setSupportNeeded(basic.getSupportNotes());
                }
                if (basic.getProjectPlanGap() != null) {
                    details.setProjectPlanGap(basic.getProjectPlanGap());
                }
                // 二、客户信息矩阵：EAV 行序列化为 JSON
                var customerInfos = eval.getCustomerInfos();
                if (customerInfos != null && !customerInfos.isEmpty()) {
                    try {
                        var rows = customerInfos.stream()
                                .map(ci -> java.util.Map.of(
                                        "roleKey", ci.getRoleKey(),
                                        "infoKey", ci.getInfoKey(),
                                        "cellValue", ci.getCellValue() != null ? ci.getCellValue() : "",
                                        "valueType", ci.getValueType().name()
                                ))
                                .toList();
                        details.setCustomerInfoJson(objectMapper.writeValueAsString(rows));
                    } catch (JsonProcessingException e) {
                        log.warn("Failed to serialize customerInfoJson for tender {}", tenderId, e);
                    }
                }
                initiationDetailsRepository.save(details);
                log.info("Created ProjectInitiationDetails from tender evaluation, project={}", saved.getId());
            });
        } catch (RuntimeException e) {
            log.warn("Failed to copy evaluation data to initiation for tender {}, project {}",
                    tenderId, saved.getId(), e);
        }
