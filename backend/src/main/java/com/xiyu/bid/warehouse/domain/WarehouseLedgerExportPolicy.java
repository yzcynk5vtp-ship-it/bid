...
/**
 * 纯核心：仓库台账导出策略 — 19 列精简版（不含附件、不含系统字段）。
 * 用于 4.4.1.6 日常台账导出场景。
 */
public class WarehouseLedgerExportPolicy {
    ...
    public static final int COL_START = 8;
    public static final int COL_END = 9;
    public static final int COL_LESSOR = 10;
    public static final int COL_LESSEE = 11;
    public static final int COL_INVOICE_PERIOD = 12;
    public static final int COL_HAS_CERT = 13;
    public static final int COL_HAS_INVOICE = 14;
    public static final int COL_HAS_PHOTOS = 15;
    public static final int COL_CLOSE_PLAN = 16;
    public static final int COL_STATUS = 17;
    public static final int COL_REMARKS = 18;

    public static final int HEADER_COUNT = 19;

    private static final Set<Integer> BASIC_INFO = Set.of(
            COL_NAME, COL_TYPE, COL_REGION, COL_PROVINCE, COL_ADDRESS, COL_AREA, COL_CONTACT, COL_REMARKS);
    private static final Set<Integer> LEASE_INFO = Set.of(
            COL_START, COL_END, COL_LESSOR, COL_LESSEE, COL_INVOICE_PERIOD, COL_CLOSE_PLAN);
    private static final Set<Integer> DOC_STATUS = Set.of(
            COL_HAS_CERT, COL_HAS_INVOICE, COL_HAS_PHOTOS, COL_STATUS);

    public static final String[] HEADERS = {
            "序号", "仓库名称", "仓库类型", "所属区域", "所在省份", "具体地址", "面积", "区域联系人",
            "开始时间", "结束时间", "出租方", "承租方", "发票租期",
            "是否有产权证", "是否有发票", "是否有内外照片", "关仓计划", "状态", "备注"
    };
    ...
    public static List<String[]> buildRows(List<WarehouseEntity> entities, Set<Section> enabled) {
        ...
        for (WarehouseEntity e : entities) {
            ...
            for (int i = 0; i < HEADER_COUNT; i++) {
                filtered[i] = cols.contains(i) ? all[i] : "";
            }
            rows.add(filtered);
        }
        return rows;
    ...
    public static String[] getFullHeaders() {
    ...
    private static String[] fullRow(WarehouseEntity e, int seq) {
        return new String[]{
                String.valueOf(seq),
                nvl(e.getName()),
    ...
    private static String formatInvoicePeriod(WarehouseEntity e) {
        LocalDate s = e.getInvoicePeriodStart();
        LocalDate t = e.getInvoicePeriodEnd();
        if (s == null && t == null) {
            return nvl(e.getInvoicePeriod());
        }
    ...
    public static long daysToExpiry(WarehouseEntity e) {
