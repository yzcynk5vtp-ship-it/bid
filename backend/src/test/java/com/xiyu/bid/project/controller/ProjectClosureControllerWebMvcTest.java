...
// Output: 验证 Closure 接口契约（@Valid + DTO + comment 字段）的反序列化/校验行为
...
/**
 * ProjectClosureController 审批接口契约测试。参照 docs/architecture/approval-contract.md。
...
class ProjectClosureControllerWebMvcTest {
    ...
    void reject_validComment_returns200() throws Exception {
        when(authService.resolveUserIdByUsername("user")).thenReturn(1L);
        when(service.rejectClosure(eq(1L), eq("不行"), eq(1L)))
                .thenReturn(ClosureDTO.builder().id(10L).projectId(1L).stageLocked(false)
                        .closedAt(LocalDateTime.now()).build());
        mockMvc.perform(post("/api/projects/1/closure/reject")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"comment\":\"不行\"}"))
