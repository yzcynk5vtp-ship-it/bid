...
/**
 * RoleCodeCachePort 内存实现，供集成测试复用。
 *
 ...
 *
 * <p>提供方：各测试类通过 {@code @Import(InMemoryRoleCodeCachePortConfig.class)} 引入。
 * 注入点声明类型为 {@link InMemoryRoleCodeCachePort}（而非接口），Spring 按类型注入具体实现，
 * 避免强制类型转换。
 */
@TestConfiguration
public class InMemoryRoleCodeCachePortConfig {
     ...
     * </ul>
     *
     * <p>使用注意：注入点声明为 {@link InMemoryRoleCodeCachePort} 类型，
     * 直接调用 {@code port.put()} / {@code port.clear()}，无需 asInMemory() 强制转换。
     */
