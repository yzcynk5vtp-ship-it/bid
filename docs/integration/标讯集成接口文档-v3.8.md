...
**回调地址**：由外部系统提供，通过 `webhook.crm.url` 配置。
...
| 字段 | 类型 | 说明 |
|------|------|------|
| event | String | 事件类型，固定值 `tender.status_changed` |
| tenderId | Long | 标讯 ID |
| sourceId | String | 来源系统的数据唯一 ID（v3.5 变更：从 externalId 提取 sourceId，与标讯推送接口保持一致） |
| oldStatus | String | 变更前状态 |
| newStatus | String | 变更后状态 |
| title | String | 标讯标题 |
| occurredAt | String | 发生时间（ISO 8601） |
| operatorId | Long | 操作人 ID（v3.5 新增） |
| operatorName | String | 操作人姓名（v3.5 新增） |
| abandonReason | String | 弃标原因（仅弃标时返回，v3.5 新增） |
| recommendation | Object | 评估建议（v3.5 新增），含 `shouldBid`（Boolean）和 `reason`（String） |
...
```json
{
  ...
  "newStatus": "BIDDING",
  "title": "西域集团2026年度MRO物料采购招标项目",
  "occurredAt": "2026-06-17T14:30:00",
  "operatorId": 1,
  "operatorName": "张经理",
  "recommendation": {
    "shouldBid": true,
