...
## CO-400 Round 5：资源管理-账户管理-详情页编辑按钮权限控制（2026-06-30）

### 问题背景
- Linear: CO-400（Bug: 资源管理-账户管理-新增账户信息保存后部分字段值丢失）
- 前 4 轮修复集中在列表页（`AccountRowActions.vue`、`rowActionsFor`、`resolveAccountActions`），但用户最新反馈"在账户详情页可看到编辑按钮"，点击后 403。
- 后端 `PUT /api/platform/accounts/{id}` 的 `@PreAuthorize("hasAnyRole('ADMIN', 'MANAGER')")` 是正确的防御纵深（403 是预期行为），问题在前端：详情页对话框 footer 的"编辑"按钮无条件渲染。

### 根因定位（严格遵循 lessons-learned.md §23 全链路日志排查 SOP）
1. **Step 1-2 日志**：`trae/.runtime/dev-services/backend.log` 显示服务 08:30:06 已关闭，日志无 403 记录（Spring Security 方法拦截器在进入 GlobalExceptionHandler 前就拒绝，所以日志看不到）。
2. **Step 4 禁止乱猜**：改用代码证据链。对比发现：
   - 列表页 `AccountRowActions.vue:2` 正确使用 `v-if="actions.edit"`
   - 详情页 `AccountDetailDialog.vue:59` 的"编辑"按钮 `<el-button type="primary" @click="$emit('edit')">编辑</el-button>` **无任何 v-if 条件**，对所有用户无条件渲染。
- 关键洞察：前 4 轮全部聚焦列表页入口，完全漏掉详情页对话框 footer 这个第二入口。

### 实现决策
1. **只改前端，不改后端**：后端 `@PreAuthorize` 是正确的防御纵深，保留。前端按钮控制是 UX 层，避免用户点击后撞 403 死胡同。
2. **复用现有 `resolveAccountActions`**：不新增权限逻辑，确保详情页与列表页使用同一套权限矩阵（对称设计，§24）。
3. **`actions` prop 安全默认**：`default: () => ({})` 遵循"默认拒绝"原则——未传 actions 时所有按钮不渲染，避免重蹈"无条件渲染"覆辙。
4. **未改"登记归还"按钮**：遵循"只改与本 issue 直接相关代码"原则，该按钮不在本轮修复范围。

### 行预算问题与解决
- `Account.vue` 原 300 行（已踩 300 行硬上限）。新增 `:actions="..."` 透传后 `npm run build` 报 `[CROSSED_LIMIT] src/views/Resource/Account.vue (300 -> 304 lines)`。
- 迭代过程：(1) 删 3 行注释 → 301 行仍超 (2) 最终方案：删除 `detailActions` computed，模板内直接内联 `rowActionsFor(currentAccountDetail || {})` → 300 行（净 0 变化）。
- 教训：棘轮式门禁下，已踩上限的文件只能"以等量代码换等量代码"。

### 修改文件范围（3 个文件）

| 文件 | 改动 |
|---|---|
| `src/views/Resource/AccountDetailDialog.vue` | 新增 `actions` prop（默认 `{}`，默认拒绝）；"编辑"按钮加 `v-if="actions.edit"` |
| `src/views/Resource/Account.vue` | 模板 `AccountDetailDialog` 透传 `:actions="rowActionsFor(currentAccountDetail || {})"`（内联，不新增 computed） |
| `src/views/Resource/__tests__/AccountDetailDialog.spec.js` | `mountDialog` 支持传入 actions；新增 2 个测试用例覆盖 `edit: false` 隐藏 / `edit: true` 显示 |

### 未做的事（避免越界）
- 未改后端 `PlatformAccountController.java`（`@PreAuthorize` 是正确的防御纵深）
- 未改 `accountActions.js` 的 `resolveAccountActions`（权限矩阵已正确，复用即可）
- 未改 `AccountRowActions.vue`（列表页已正确使用 `v-if`）
- 未改"登记归还"按钮（不在本轮范围）

### 验证证据
- TDD Red：2 个新测试，1 个失败（编辑按钮无条件渲染）✅ 符合预期
- TDD Green：加 `actions` prop + `v-if` 后 10/10 测试通过 ✅
- `npm run build`：exit 0（行预算通过，300 → 300 净 0）✅
- `npm run test:unit`：1836 passed | 2 failed | 1 skipped
  - 2 个失败经 `git stash` 验证为 `origin/main` baseline 同样失败（`scripts/start-env-detection.spec.js`、`scripts/sidecar-dev-services.spec.js`）
  - 属 zcode worktree 环境检测已知基线问题（§22），非本次改动引入

### 建议新增 lessons-learned 条目
**"Bug 修复必须审视同一业务动作的所有 UI 入口"**
- 同一业务动作（如"编辑账户"）可能存在多个 UI 入口（列表页行操作 + 详情页 footer 按钮）。
- 修复时若只改一个入口，用户会从另一个未修入口绕过权限控制撞 403。
- 经验：修复权限 Bug 前，先全局搜索该业务动作的所有触发点（`@click="$emit('edit')"`、`handleEdit`、`rowActionsFor` 等），逐一核对权限守卫。

---

## CO-415: 详情页"登记归还"按钮 actions.return 守卫 + 后端 returnAccount 联系人豁免（对称修复）

### 问题背景
- Linear: CO-415（follow-up of CO-400 round5，同系列对称性 bug）
- CO-400 round5 思维链 Review 时发现：详情页"编辑"按钮已加 `v-if="actions.edit"` 守卫，但同 footer 的"登记归还"按钮 `v-if="data?.status === 'in_use'"` 仅判状态、**缺少 `actions.return` 守卫**，与列表页 `AccountRowActions.vue:5` 的 `v-if="actions.return"` 不对称。
- 更深的问题在后端：`returnAccount` 端点 `@PreAuthorize("hasAnyRole('ADMIN', 'MANAGER')")` 与 round4 修复前的 `getPassword` 同病——bid-Team 在 `ROLES_WITHOUT_LEGACY_ROLE_COMPAT` 列表中（`RoleProfileCatalog.java:76`），`UserDetailsServiceImpl.skipLegacyCompat=true` 导致**不发 `ROLE_MANAGER` authority**，所以 `hasAnyRole('ADMIN','MANAGER')` 一刀切拦截**所有 bid-Team（包括绑定联系人）**，与 `resolveAccountActions` 给联系人放行 `return:isInUse` 的授权语义冲突。

### 根因定位（复用 CO-400 round5 的代码证据链）
1. **前端对称缺口**：`AccountDetailDialog.vue:58` "登记归还"按钮仅判 `status`，未走 `actions.return`；而列表页 `AccountRowActions.vue:5` 正确使用 `v-if="actions.return"`。同一业务动作两个入口，守卫不一致。
2. **后端一刀切**：`PlatformAccountController` 的 `/return` 与 `/return-with-password` 方法级 `@PreAuthorize("hasAnyRole('ADMIN', 'MANAGER')")` 在 Spring Security 方法拦截器层直接 403，**绑定联系人也无法归还**。Service 层 `returnAccount` 收到 `currentUser` 但**完全未做权限校验**——唯一的防线就是 Controller 的 `@PreAuthorize`，而它正好误拦联系人。
3. **Red 阶段验证根因**：后端 Red 测试 `returnAccount_bidTeamNotContactPerson_throws` 期望抛 `IllegalStateException`，实际得到 NPE（`account` 在 `PlatformAccountMapper.toDTO` 中为 null）——因为 Service **根本没校验**，非联系人一路执行到 returnToPool 后 mapper 才崩。这精确证实"Service 无校验"是真实漏洞，不是测试写错。

### 实现决策
1. **完全对称于 getPassword 范式**（CO-400 round4 已验证的模式）：
   - Controller 去掉方法级 `@PreAuthorize`，仅保留类级 `@PreAuthorize("hasAuthority('resource')")`（`PlatformAccountController.java:55`）作为防御纵深。
   - Service 层加联系人豁免：`PlatformAccountViewerPolicy.checkCanReturnAccount(roleCode, account, currentUser)`。
2. **Policy 纯静态**（FP-Java Contract）：`canReturnAccount` / `checkCanReturnAccount` 不依赖 Spring 上下文，`roleCode` 由 Service 通过 `EffectiveRoleResolver.resolveRoleCode` 解析后传入，保持 Policy 可单测。复用 `isPrivilegedRole` / `isBidTeamRole` 与 `PlatformAccountContactMatcher.isContactPerson`，与 `canViewPassword` 授权语义完全一致。
3. **行预算策略**：`PlatformAccountService.java` 原 308 行（已超 300 硬上限，棘轮=308）。
   - 迭代 1：抽 `assertCanReturnAccount` 私有方法 → 333（仍超）
   - 迭代 2：逻辑移到 `PlatformAccountViewerPolicy.canReturnAccount` → 333（仍超）
   - 迭代 3：压缩 javadoc → 310（仍超）
   - 迭代 4：再抽 `checkCanReturnAccount`（throws 版本），Service 内联调用，删私有方法 → 302（净 -6，棘轮允许下降）✅
   - 教训：超限文件要"净下降"才能过棘轮，把可复用逻辑外移到 Policy（纯静态、不超限）是双赢。

### 修改文件范围（6 个文件）

| 文件 | 改动 |
|---|---|
| `src/views/Resource/AccountDetailDialog.vue` | "登记归还"按钮 `v-if` 由 `data?.status === 'in_use'` 改为 `actions.return`（与"编辑"按钮对称） |
| `src/views/Resource/__tests__/AccountDetailDialog.spec.js` | +2 测试：非联系人隐藏 / 联系人显示登记归还按钮（12/12 pass） |
| `backend/.../controller/PlatformAccountController.java` | `/return` 与 `/return-with-password` 去掉方法级 `@PreAuthorize`（类级 `hasAuthority('resource')` 保留） |
| `backend/.../service/PlatformAccountService.java` | 两处 `returnAccount` 加 `checkCanReturnAccount` 调用；压缩 javadoc（308→302，-6） |
| `backend/.../service/PlatformAccountViewerPolicy.java` | +`canReturnAccount` / `checkCanReturnAccount` 纯静态方法（30→47） |
| `backend/.../test/.../PlatformAccountServiceTest.java` | +4 测试覆盖联系人/非联系人 × 有密码/无密码两条路径（31/31 pass） |
| `backend/.../test/.../PlatformAccountControllerSecurityTest.java` | +3 测试覆盖 admin/bid-Team/无 resource 权限三类调用方（9/9 pass） |

### 未做的事（避免越界）
- 未改 `accountActions.js` 的 `resolveAccountActions`（权限矩阵已正确给联系人放行 `return:isInUse`，复用即可）
- 未改 `AccountRowActions.vue`（列表页已正确使用 `v-if="actions.return"`）
- 未改 `Account.vue`（round5 已透传 `:actions`，本轮复用）
- 未顺手重构 `returnAccount` 的业务逻辑（仅加权限校验，遵循"只改直接相关代码"）

### 验证证据
- 前端 TDD Red→Green：12/12 pass
- 后端 TDD Red→Green：40/40 pass（31 Service + 9 Security），BUILD SUCCESS
- `npm run build`：全部门禁通过（`line-budget check passed. guarded_changes=4`、inlined-composables、composable-placement、token-governance、front-data-boundaries 等 11 项）
- 推送门禁：13 通过 / 0 失败 / 6 跳过
- 提交：`1f94fe07d`（7 files changed, +167/-15）

### 建议新增 lessons-learned 条目
**"权限 Bug 修复必须前后端对称审查 + 复用已验证范式"**
- 前端加 `v-if` 守卫只是 UX 层，后端 `@PreAuthorize` 一刀切才是真实拦截点。修前端不修后端 = 联系人看到按钮点了仍 403；修后端不修前端 = 联系人没按钮但 API 可达（不一致）。
- 已有验证过的范式（如 getPassword 的"Controller 去 @PreAuthorize + Service 加联系人豁免"）应作为模板复用，避免重新设计引入新风险。
- 经验：审查权限 Bug 时，用 `git log -S "@PreAuthorize" -- <Controller>` 找出同类端点，逐一对照 Service 层是否做了细粒度校验。

---

## CO-409 CA信息管理模块投标专员操作项权限矩阵对齐 (2026-06-30)

### 问题口径
- 投标专员（bid-Team）无法访问 CA 管理模块任何接口，所有 18 个接口返回 403。
- 根因三层：① Controller 类级 `hasAnyRole('ADMIN','MANAGER')` 全拦；② bid-Team 在 `ROLES_WITHOUT_LEGACY_ROLE_COMPAT` 中，不发 ROLE_MANAGER；③ menuPermissions 缺 `resource-ca`，侧边栏入口隐藏。

### 决策与权衡
1. **复刻 §28 范式**：对称于 `PlatformAccountViewerPolicy.canManageAccount`/`canReturnAccount`——Controller 类级 `hasAuthority('resource')` + Service 层保管员校验 + `EffectiveRoleResolver` 解析角色码。
2. **架构合规修正**：初版把 `UserRepository`/`User` 依赖放在 strict-module 的 `resources.controller`，被 ArchitectureTest 拦截。修正为把 `resolveUser` 下沉到 Service 层（对称于同模块 `CaBorrowService.resolveUser`），Controller 只传 `UserDetails`。
3. **需求差距纠偏（Phase 3 自审发现）**：用户初选"选项A 最小化"（只放开新增/批量导入按钮，保持 5 列简化视图）。自审发现后端 `update`/`deactivate` 已放开但前端简化视图无编辑/下架入口，形成"后端开权限但 UI 调不到"的不对称。经用户确认改为"补齐 issue 完整矩阵"：bid-Team 进 `isManagerView`（10 列+统计+筛选），操作项按保管员差异化（`canManageRow`/`canBorrowRow`）。
4. **canBorrowRow 语义**：保管员不可借用自己保管的 CA（对称 issue 矩阵「借用：保管员❌ 非保管员✅」），非保管员可借用。
5. **pending-approvals 保留 admin-only**：按权限矩阵，待审批列表仅管理员可见，不对 bid-Team 放开。
6. **issue 描述过时项**：路由 meta `permissionKeys` 和侧边栏 `permissionKeys: ['resource-ca']` 实际已存在，issue 说"需补"是基于旧代码的描述，本次无需改动。

### 改动清单

| 文件 | 改动 |
|---|---|
| `backend/.../entity/RoleProfileCatalog.java` | bid-Team menuPermissions 补 `resource-ca` 权限点 |
| `backend/.../resources/controller/CaCertificateController.java` | 类级 `hasAnyRole('ADMIN','MANAGER')` → `hasAuthority('resource')`；写操作方法级加 `or hasAuthority('ROLE_BID_TEAM')`；deactivate 传 `UserDetails`；pending-approvals 保留 admin-only |
| `backend/.../resources/service/CaCertificateService.java` | `deactivate` 下沉保管员差异化校验（管理员任意/投标专员仅自己保管）+ 内部 `resolveUser` + 新增 `UserRepository` 依赖 |
| `backend/.../test/.../RoleProfileCatalogTest.java` | +1 测试断言 resource-ca 权限点 |
| `backend/.../test/.../CaCertificateServiceTest.java` | **新建** 4 测试（admin 任意/bid-Team 保管员可/bid-Team 非保管员拒/null 拒） |
| `src/views/Resource/CAManagement.vue` | `isManagerView` 纳入 bid-Team；新增 `canCreate`/`canManageRow`/`canBorrowRow`/`currentUserId`；操作列按钮按保管员差异化 |
| `src/views/Resource/__tests__/CAManagement.spec.js` | bid-Team 测试块改为完整矩阵（6 测试：canCreate/isManagerView/新增按钮/统计卡片/canManageRow/canBorrowRow） |

### 验证证据
- 后端：`CaCertificateServiceTest` 4/4 + `RoleProfileCatalogTest` 1/1 + `ArchitectureTest` 25/25（含 strict-module controller 规则）+ `FPJavaArchitectureTest` 8/8 + `RoleProfileServicePersistenceTest` 2/2 + `RoleProfileBootstrapArchitectureTest` 1/1，BUILD SUCCESS
- 前端：`CAManagement.spec.js` 18/18（含 6 个 bid-Team 完整矩阵用例）
- `npm run check:line-budgets`：passed (guarded_changes=2)
- `npm run build`：✓ built in 8.85s
- preflight-self-review.sh：通过

### 关键经验
- **strict-module Controller 不能依赖 `..entity..`/`..repository..`**：ArchitectureTest 会拦截。UserDetails→User 的解析必须下沉到 Service 层。
- **Phase 3 自审要对照 issue 原始矩阵**：不能只看测试绿就认为完成。选项A 虽测试全绿，但漏了 issue 要求的编辑/下架/统计卡片/高级筛选入口，形成前后端不对称。
- **authority 注入核实**：`hasAuthority('ROLE_BID_TEAM')` 的 authority 名由 `RoleProfileCatalog.toAuthorityName("bid-Team")`=`"BID_TEAM"` 生成，注入 `ROLE_BID_TEAM`，写法正确。
