<template>
  ...
  <el-dialog v-model="ctx.reviewerDialogVisible" title="添加评审人" width="500px"><el-form :model="ctx.reviewerForm" label-width="100px"><el-form-item label="评审人" required><UserPicker v-model="ctx.reviewerForm.userId" placeholder="搜索评审人（姓名/工号/拼音）" style="width: 100%;" /></el-form-item><el-form-item label="评审角色" required><el-select v-model="ctx.reviewerForm.role" style="width: 100%;"><el-option label="技术评审" value="tech" /><el-option label="商务评审" value="business" /></el-select></el-form-item></el-form><template #footer><el-button @click="ctx.reviewerDialogVisible = false">取消</el-button><el-button type="primary" @click="ctx.handleConfirmAddReviewer">确定</el-button></template></el-dialog>
</template>
