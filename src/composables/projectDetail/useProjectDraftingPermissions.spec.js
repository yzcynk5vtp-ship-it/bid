...
// 仅管理员/组长 + 当前项目投标负责人可选择标书审核人
function computeCanSelectReviewer(role, opts = {}) {
  const group = resolveDraftingRoleGroup(role)
  if (group === 'admin_lead') return true
  if (role !== 'bid-projectLeader' || !opts.currentUserId) return false
  return !!(opts.primaryLeadId && String(opts.primaryLeadId) === String(opts.currentUserId))
}
...
describe('canSelectReviewer — 选择标书审核人权限（仅 admin_lead + 项目主负责人）', () => {
  it.each([
    ['admin', true],
    ['/bidAdmin', true],
    ['bid-TeamLeader', true],
  ...
  it('bid-Team 匹配 secondaryLeadId 也不可择审核人', () => {
    expect(computeCanSelectReviewer('bid-Team', { currentUserId: 20, primaryLeadId: 10, secondaryLeadId: 20 })).toBe(false)
  })
})
...
describe('CO-355: reactive(composable) 真实调用（解包 + 项目级 lead 匹配）', () => {
  ...
  it('canSelectReviewer 解包为 boolean 且仅 admin_lead / 项目主负责人为 true', () => {
    ...
    expect(permWith('bid-Team', { primaryLeadId: 1, secondaryLeadId: 1, currentUserId: 1 }).canSelectReviewer).toBe(false)
