...
describe('TenderEvaluationForm — instance-level permission matrix (V130 3-section)', () => {
  ...
  it('canFill=true + status=null → editable + save-draft/submit buttons + NO bid/abandon', () => {
    const wrapper = makeWrapper({ canFill: true, canDecide: false, evaluation: null })
  ...
  it('canFill=true + status=DRAFT → editable + save-draft/submit buttons + NO bid/abandon', () => {
    const wrapper = makeWrapper({
  ...
  it('canFill=true + status=SUBMITTED → read-only + NO submit', () => {
    const wrapper = makeWrapper({
  ...
  it('canFill=false + status=DRAFT → read-only + NO submit', () => {
    const wrapper = makeWrapper({
  ...
  it('canFill=false + status=null → read-only + NO submit (un-assigned tender)', () => {
    const wrapper = makeWrapper({
  ...
  it('submit event fires with full payload when canFill=true and recommendation filled', async () => {
    ...
    // recommendation.shouldBid is validated to be truthy (true or 'true' from stub)
  ...
  it('submit does NOT fire when recommendation missing', async () => {
    const wrapper = makeWrapper({ canFill: true, canDecide: false, evaluation: null })
    await fillRequiredBasicFields(wrapper)
    // Do NOT fill recommendation.shouldBid
    await findButtonByText(wrapper, '提交').trigger('click')
    await flushPromises()
  ...
  it('submit does NOT fire when plannedShortlistedCount = 0 (policy requires >= 1)', async () => {
    const wrapper = makeWrapper({ canFill: true, canDecide: false, evaluation: null })
    await fillRequiredBasicFields(wrapper)
    // Override plannedShortlistedCount to 0
