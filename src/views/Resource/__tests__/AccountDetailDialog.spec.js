...
const mountDialog = (data = mockAccountDetail, actions = undefined) =>
...
describe('AccountDetailDialog.vue — CO-389 详情字段漂移修复 + 密码字段补全', () => {
  ...
  // CO-400 round5：详情页编辑按钮必须受 actions.edit 控制，与列表页对称。
  // 根因：详情页 footer 编辑按钮原为无条件渲染，导致非绑定联系人的投标专员
  // 也能看到编辑入口，点击保存后触发后端 403（@PreAuthorize 正确拦截）。
  // 修复：编辑按钮加 v-if="actions.edit"，复用 resolveAccountActions 权限矩阵。
  it('render_hidesEditButton_whenActionsEditFalse — 非管理员非联系人不应看到编辑按钮', () => {
    // 非绑定联系人的投标专员：resolveAccountActions 返回 { borrow: true }，无 edit
    const wrapper = mountDialog(mockAccountDetail, { edit: false, borrow: true })
    const footerButtons = wrapper.findAll('button.el-button-stub')
    const editBtn = footerButtons.find(b => b.text().includes('编辑'))
    expect(editBtn).toBeUndefined()
  })

  it('render_showsEditButton_whenActionsEditTrue — 管理员/绑定联系人应看到编辑按钮', () => {
    // 管理员：resolveAccountActions 返回 { edit: true, ... }
    const wrapper = mountDialog(mockAccountDetail, { edit: true })
    const footerButtons = wrapper.findAll('button.el-button-stub')
    const editBtn = footerButtons.find(b => b.text().includes('编辑'))
    expect(editBtn).toBeDefined()
  })

  // CO-415：详情页"登记归还"按钮必须受 actions.return 控制，与列表页对称。
  // 根因：原 v-if="data?.status === 'in_use'" 只看状态不看权限，导致
  // 非绑定联系人的投标专员也能看到登记归还入口，点击后端 returnAccount 403。
  it('render_hidesReturnButton_whenActionsReturnFalse — 非联系人不应看到登记归还按钮', () => {
    // 非绑定联系人的投标专员：resolveAccountActions 返回 { borrow: true }，无 return
    const inUseAccount = { ...mockAccountDetail, status: 'in_use' }
    const wrapper = mountDialog(inUseAccount, { borrow: true })
    const footerButtons = wrapper.findAll('button.el-button-stub')
    const returnBtn = footerButtons.find(b => b.text().includes('登记归还'))
    expect(returnBtn).toBeUndefined()
  })

  it('render_showsReturnButton_whenActionsReturnTrue — 管理员/绑定联系人应看到登记归还按钮', () => {
    // 管理员/绑定联系人 + in_use：resolveAccountActions 返回 { edit, return: true, ... }
    const inUseAccount = { ...mockAccountDetail, status: 'in_use' }
    const wrapper = mountDialog(inUseAccount, { edit: true, return: true, takeDown: true })
    const footerButtons = wrapper.findAll('button.el-button-stub')
    const returnBtn = footerButtons.find(b => b.text().includes('登记归还'))
