...
describe('CADetailDrawer.vue — CO-406 详情形态对齐 AccountDetailDialog（居中弹窗）', () => {
  ...
  // CO-433：在库（IN_STOCK）状态不应显示登记归还按钮
  it('render_noReturnButtonWhenInStock — 在库状态不显示登记归还按钮', async () => {
    const wrapper = await mountComponent({ isManager: true })
    await flushPromises()
  ...
  // CO-433：已借出（BORROWED）状态显示登记归还按钮
  it('render_returnButtonWhenBorrowed — 已借出状态显示登记归还按钮', async () => {
  ...
  // CO-433：已借出状态点击登记归还按钮 emit return 事件
  it('emit_returnWhenBorrowedClicksReturn — 已借出状态点击登记归还 emit return 事件', async () => {
    const borrowedCa = { ...mockCa, borrowStatus: 'BORROWED', borrowStatusLabel: '已借出', currentBorrowerName: '王五' }
    const wrapper = await mountComponent({ isManager: true, ca: borrowedCa })
    await flushPromises()
